...
You can choose where to store the dtfn files with `--datadir`. For this guide, we will use `~/.dtfn`, which is a reasonable default in most cases.

Before you can run dtfn you need to initialise tendermint and dtfn with their respective genesis states.
...
which will also invoke `tendermint init --home ~/.dtfn/tendermint`. You can prevent Tendermint from
...
For simplicity, we'll have dtfn start tendermint as a subprocess with the
